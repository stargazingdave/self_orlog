...
def melee_aggressive_favor(
    s: GameState, opp_id: PlayerId, rand: Randomizer, p_action: float
):
    player = get_player(s, opp_id)
    # Prefer Thor
    thor_favor = get_god_favor_def(GodFavorId.THOR_STRIKE)
    l1_offset = 0  # index of Thor L1 in action layout
    l2_offset = 1  # index of Thor L2 in action layout
    l3_offset = 2  # index of Thor L3 in action layout
    if rand.random() < p_action:
        if player.tokens >= thor_favor.levels[2].cost:
            return FAVOR_START + l3_offset  # Thor L3
        elif player.tokens >= thor_favor.levels[1].cost:
            return FAVOR_START + l2_offset  # Thor L2
        elif player.tokens >= thor_favor.levels[0].cost:
            return FAVOR_START + l1_offset  # Thor L1
    return FAVOR_SKIP
